...
# Author: ZHOU Yuqi
# The script is designed to run the tested algorithm program and use BAliBASE reference 9 to evaluate it.

command=$1

curdir=$(pwd)
dirA="$curdir/R9"
dirB="$curdir/R9results"

# Create directory B if it doesn't already exist.
mkdir -p "$dirB"

# Loop over subdirectories in dirA that begin with "RV"
for subdir in "$dirA"/RV*; do
    ...
    if [ -d "$subdir" ]; then
        ...
        for tfa_file in "$subdir"/*.in_tfa; do
            ...
            if [ -e "$tfa_file" ]; then
                ...
                "$curdir"/R9/BAliBASE_R9/src/bali_score_elm "$subdir/${base}.xml" "$out_dir/${base}.msf" | tail -n 1 >> "$out_dir1/log.txt"
